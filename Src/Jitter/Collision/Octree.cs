...
namespace Jitter.Collision
{
    ...
    public class Octree
    {
        ...
        public void BuildOctree()
        {
            ...
            triBoxes = new BBox[tris.Length];

            // create an infinite size root box
            rootNodeBox = new BBox(new Vector3(float.PositiveInfinity, float.PositiveInfinity, float.PositiveInfinity),
            ...
            BBox[] children = new BBox[8];
            for (int triNum = 0; triNum < tris.Length; triNum++)
            {
                int nodeIndex = 0;
                BBox box = rootNodeBox;

                while (box.Contains(ref triBoxes[triNum]) == BBox.ContainmentType.Contains)
                {
                    int childCon = -1;
                    for (int i = 0; i < 8; ++i)
                    {
                        CreateAABox(ref box, (EChild)i,out children[i]);
                        if (children[i].Contains(ref triBoxes[triNum]) == BBox.ContainmentType.Contains)
                        {
        ...
        public int GetTrianglesIntersectingtAABox(List<int> triangles, ref BBox testBox)
        {
            if (nodes.Length == 0)
                return 0;
            int curStackIndex = 0;
            ...
            while (curStackIndex < endStackIndex)
            {
                ...
                curStackIndex++;
                if (nodes[nodeIndex].box.Contains(ref testBox) != BBox.ContainmentType.Disjoint)
                {
                    for (int i = 0; i < nodes[nodeIndex].triIndices.Length; ++i)
                    {
                        if (triBoxes[nodes[nodeIndex].triIndices[i]].Contains(ref testBox) != BBox.ContainmentType.Disjoint)
                        {
