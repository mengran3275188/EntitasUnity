    ...
    public class CompoundShape : Multishape
    {
        ...
        public struct TransformedShape
        {
            ...
            public Matrix3x3 Orientation
            {
                ...
                set { orientation = value; Matrix3x3.Transpose(ref orientation, out invOrientation); UpdateBoundingBox(); }
            }
            ...
            public TransformedShape(Shape shape, Matrix3x3 orientation, Vector3 position)
            {
        ...
        {
            base.inertia = Matrix3x3.Zero;
            base.mass = 0.0f;

            for (int i = 0; i < Shapes.Length; i++)
            {
                Matrix3x3 currentInertia = Shapes[i].InverseOrientation * Shapes[i].Shape.Inertia * Shapes[i].Orientation;
        ...
        public override int Prepare(ref BBox box)
        {
            ...
            for (int i = 0; i < shapes.Length; i++)
            {
                if (shapes[i].boundingBox.Contains(ref box) != BBox.ContainmentType.Disjoint)
