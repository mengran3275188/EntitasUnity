...
namespace Jitter.Collision
{
    ...
    public class DynamicTree<T>
    {
        ...
        public int AddProxy(ref BBox aabb, T userData)
        {
        ...
        public bool MoveProxy(int proxyId, ref BBox aabb, Vector3 displacement)
        {
            Debug.Assert(0 <= proxyId && proxyId < _nodeCapacity);

            Debug.Assert(_nodes[proxyId].IsLeaf());

            if (_nodes[proxyId].AABB.Contains(ref aabb) != BBox.ContainmentType.Disjoint)
            {
                return false;
            }
        ...
        public void GetFatAABB(int proxyId, out BBox fatAABB)
        {
            Debug.Assert(0 <= proxyId && proxyId < _nodeCapacity);
        ...
        public void Query(List<int> other, List<int> my, DynamicTree<T> tree)
        {
            ...
            while (stack1.Count > 0)
            {
                ...
                if (tree._nodes[nodeId2].AABB.Contains(ref _nodes[nodeId1].AABB) != BBox.ContainmentType.Disjoint)
                {
        ...
        private void InsertLeaf(int leaf)
        {
            ...
            BBox leafAABB = _nodes[leaf].AABB;
            int sibling = _root;
            while (_nodes[sibling].IsLeaf() == false)
            {
                ...
                BBox.CreateMerged(ref _nodes[sibling].AABB, ref leafAABB, out _nodes[sibling].AABB);

                _nodes[sibling].LeafCount += 1;

                float siblingArea = _nodes[sibling].AABB.Perimeter;
                BBox parentAABB = new BBox();
                ...
                if (_nodes[child1].IsLeaf())
                {
                    ...
                    BBox.CreateMerged(ref leafAABB, ref _nodes[child1].AABB, out aabb);
                ...
                if (_nodes[child2].IsLeaf())
                {
                    ...
                    BBox.CreateMerged(ref leafAABB, ref _nodes[child2].AABB, out aabb);
                ...
                //_nodes[sibling].AABB.Combine(ref leafAABB);
                BBox.CreateMerged(ref leafAABB, ref _nodes[sibling].AABB, out _nodes[sibling].AABB);
            ...
            BBox.CreateMerged(ref leafAABB, ref _nodes[sibling].AABB, out _nodes[newParent].AABB);
        ...
        private void RemoveLeaf(int leaf)
        {
            ...
            if (grandParent != NullNode)
            {
                ...
                while (parent != NullNode)
                {
                    ...
                    BBox.CreateMerged(ref _nodes[_nodes[parent].Child1].AABB,
