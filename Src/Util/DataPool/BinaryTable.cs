      ...
      try {
        using (FileStream fs = new FileStream(file, FileMode.Open, FileAccess.Read, FileShare.Read)) {
          ...
          if (header.m_Identity == c_Identity && header.m_Version == c_Version) {
            ...
            if (m_Records.Count > 0) {
              ...
              for (int i = 1; i < recordNum; ++i) {
                if (m_Records[i].Length != recordSize) {
                  LogUtil.Error("Record Size not equal, {0}!={1}({2})", recordSize, m_Records[i].Length, i);
                  haveError = true;
                }
              }
              if (haveError) {
            ...
            LogUtil.Error("Unknown binary file tag {0} or error version {1}", header.m_Identity, header.m_Version);
          }
          fs.Close();
        }
      } catch (Exception ex) {
