...
public partial class GameContext {
    ...
    public GameEntity cameraSerivceEntity { get { return GetGroup(GameMatcher.CameraSerivce).GetSingleEntity(); } }
    public Entitas.Data.CameraSerivceComponent cameraSerivce { get { return cameraSerivceEntity.cameraSerivce; } }
    public bool hasCameraSerivce { get { return cameraSerivceEntity != null; } }

    public GameEntity SetCameraSerivce(Entitas.Data.ICameraService newValue) {
        if (hasCameraSerivce) {
            throw new Entitas.EntitasException("Could not set CameraSerivce!\n" + this + " already has an entity with Entitas.Data.CameraSerivceComponent!",
                "You should check if the context already has a cameraSerivceEntity before setting it or use context.ReplaceCameraSerivce().");
        }
        var entity = CreateEntity();
        entity.AddCameraSerivce(newValue);
        return entity;
    }

    public void ReplaceCameraSerivce(Entitas.Data.ICameraService newValue) {
        var entity = cameraSerivceEntity;
        if (entity == null) {
            entity = SetCameraSerivce(newValue);
        } else {
            entity.ReplaceCameraSerivce(newValue);
        }
    }

    public void RemoveCameraSerivce() {
        cameraSerivceEntity.Destroy();
    }
}
...
public partial class GameEntity {
    ...
    public Entitas.Data.CameraSerivceComponent cameraSerivce { get { return (Entitas.Data.CameraSerivceComponent)GetComponent(GameComponentsLookup.CameraSerivce); } }
    public bool hasCameraSerivce { get { return HasComponent(GameComponentsLookup.CameraSerivce); } }

    public void AddCameraSerivce(Entitas.Data.ICameraService newValue) {
        var index = GameComponentsLookup.CameraSerivce;
        var component = CreateComponent<Entitas.Data.CameraSerivceComponent>(index);
        component.Value = newValue;
        AddComponent(index, component);
    }
    ...
    public void RemoveCameraSerivce() {
        RemoveComponent(GameComponentsLookup.CameraSerivce);
    }
}
...
public sealed partial class GameMatcher {
    ...
    static Entitas.IMatcher<GameEntity> _matcherCameraSerivce;

    public static Entitas.IMatcher<GameEntity> CameraSerivce {
        get {
            if (_matcherCameraSerivce == null) {
                var matcher = (Entitas.Matcher<GameEntity>)Entitas.Matcher<GameEntity>.AllOf(GameComponentsLookup.CameraSerivce);
                matcher.componentNames = GameComponentsLookup.componentNames;
                _matcherCameraSerivce = matcher;
            }

            return _matcherCameraSerivce;
        }
    }
}
