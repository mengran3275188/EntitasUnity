...
namespace Skill.CommonCommands
{
  ...
  /// assign(@@global,value);
  /// </summary>
  internal class AssignCommand : AbstractSkillCommand
  {
    ...
    private ISkillValue<object> m_Value = new SkillValue();
  }
  /// <summary>
  /// inc(@local,value);
  /// or
  /// inc(@@global,value);
  /// </summary>
  internal class IncCommand : AbstractSkillCommand
  {
    public override ISkillCommand Clone()
    {
      ...
      if (m_VarName.StartsWith("@@")) {
        if (null != instance.GlobalVariables) {
          if (instance.GlobalVariables.ContainsKey(m_VarName)) {
            ...
            if (oval is int) {
              int ov = SkillValueHelper.CastTo<int>(oval);
              if (m_ParamNum > 1 && m_Value.HaveValue) {
                int v = SkillValueHelper.CastTo<int>(m_Value.Value);
                ov += v;
              ...
              float ov = SkillValueHelper.CastTo<float>(oval);
              if (m_ParamNum > 1 && m_Value.HaveValue) {
                float v = SkillValueHelper.CastTo<float>(m_Value.Value);
                ov += v;
                instance.GlobalVariables[m_VarName] = ov;
              } else {
                ++ov;
  ...
  /// dec(@@global,value);
  /// </summary>
  internal class DecCommand : AbstractSkillCommand
  {
    public override ISkillCommand Clone()
    {
  ...
  /// propset(name,value);
  /// </summary>
  internal class PropSetCommand : AbstractSkillCommand
  {
    public override ISkillCommand Clone()
    {
  ...
  /// </summary>
  internal class ListSetCommand : AbstractSkillCommand
  {
    public override ISkillCommand Clone()
    {
